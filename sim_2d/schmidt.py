        ...
        if len(self.landmark_seq_in_state) > 0:
            ...
            if SEQ_UPDATE:
                ...
                for i in range(observation_num):
                    ...
                    cov_ab = np.identity(5)
                    cov_ab[0:3, 0:3] = self.cov[0:3, 0:3]
                    cov_ab[3:5, 3:5] = self.cov[2*ids+3:2*(ids+1)+3, 2*ids+3:2*(ids+1)+3]
                    # cov_ab[0:3, 3:5] = self.cov[0:3, 2*ids+3:2*(ids+1)+3]
                    # cov_ab[3:5, 0:3] = self.cov[2*ids+3:2*(ids+1)+3, 0:3]
                    
                    innovation_inv = np.linalg.inv(H @ cov_ab @ H.T + LIDAR_SIGMA**2 * np.identity(2))
                    kalman_gain = cov_ab @ H.T @ innovation_inv
                    self.xyt[0:3] = self.xyt[0:3] + (kalman_gain @ dz)[0:3]
                    cov_ab = (np.identity(5) - kalman_gain @ H) @ cov_ab
                    
                    self.cov[0:3, 0:3] = cov_ab[0:3, 0:3]
                    self.cov[2*ids+3:2*(ids+1)+3, 0:3] = cov_ab[3:5, 0:3]
                    self.cov[0:3, 2*ids+3:2*(ids+1)+3] = cov_ab[0:3, 3:5]
            else:
                # batch update
                pass
