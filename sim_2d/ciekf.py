...
class CI_EKF():
        ...
        if len(self.landmark_seq_in_state) > 0:
            ...
            if SEQ_UPDATE:
                ...
                for i in range(observation_num):
                    ...
                    omega = self.optimize_omega_dml('trace', cov_x, cov_f, Hx, Hf, R)

                    P = np.linalg.inv(omega * np.linalg.inv(cov_x) + (1.0-omega) * (Hx.T @ np.linalg.inv(Hf @ cov_f @ Hf.T + (1.0 - omega) * R) @ Hx))
                    K = 1.0 / omega * cov_x @ Hx.T @ np.linalg.inv(1.0 / omega * Hx @ cov_x @ Hx.T + 1.0 / (1.0 - omega) * Hf @ cov_f @ Hf.T + R)
    ...
    def optimize_omega_dml(self, criterion, cov_a, cov_b, H_a, H_b, R):
        def optimize_fn(omega):
            
            info_a = omega * np.linalg.inv(cov_a)

            if True:
                info_b = (1 - omega) * H_a.T @ np.linalg.inv(H_b @ cov_b @ H_b.T + (1 - omega) * R) @ H_a
            else:
                info_b = (1 - omega) * H_a.T @ np.linalg.inv(H_b @ cov_b @ H_b.T + R) @ H_a
