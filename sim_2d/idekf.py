...
class ID_EKF():
    ...
    def prediction(self, t):
        ...
        self.F[0:2, 2] = self.J @ (dataset['gt']['p'][idx+1] - dataset['gt']['p'][idx]) # TODO
    ...
    def absolute_observation(self, t):
        ...
        landmark_observation = dataset['measurement'][idx_h]
        ...
        if len(self.landmark_seq_in_state) > 0:
            ...
            if SEQ_UPDATE:
                ...
                for i in range(observation_num):
                    ...
                    H = np.zeros((2, dim))
                    H[:, 2] = - self.J @ (dataset['gt']['pt'][idx] - dataset['gt']['p'][idx_h])
                    H[:, 0:2] = - np.identity(2)
                    H[:, 2*ids+3:2*(ids+1)+3] = np.identity(2)

                    H = rot_mtx(dataset['gt']['psi'][idx_h]).T @ H 
                ...
                for i in range(observation_num):
                    ...
                    H[2*i:2*(i+1), 2] = - self.J @ (dataset['gt']['pt'][idx] - dataset['gt']['p'][idx_h])
                    H[2*i:2*(i+1), 0:2] = - np.identity(2)
                    H[2*i:2*(i+1), 2*ids+3:2*(ids+1)+3] = np.identity(2)
                    H[2*i:2*(i+1), :] = rot_mtx(dataset['gt']['psi'][idx_h]).T @ H[2*i:2*(i+1), :]
